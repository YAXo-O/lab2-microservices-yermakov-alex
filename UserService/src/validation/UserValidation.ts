import { body, query } from 'express-validator';

export default class UserValidation {
	public static create() {
		return [
			body('login', 'Login is required string').exists().isString(),
			body('hash', 'Hash is required field').exists().isString(),
		];
	}

	public static get() {
		return [
			query('page', 'Page is optional integer').optional().isInt(),
			query('id', 'Id is optional uuid v4 string').optional().isUUID(4),
		];
	}

	public static update() {
		return [
			body('id', 'Id is required uuid v4 string').exists().isUUID(4),
			body('login', 'Login is optional uuid v4 string').optional().isString(),
			body('hash', 'Hash is optional string').optional().isString(),
		];
	}

	public static delete() {
		return [
			query('id', 'Session id is required and should be uuid v4 string').exists().isUUID(4),
		];
	}
}
